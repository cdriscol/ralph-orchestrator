...
pub fn detect_wave_events(
    ...
    // v1: Take the first wave_id (deterministic via sorting)
    let mut wave_ids: Vec<&&str> = wave_groups.keys().collect();
    wave_ids.sort();
    let wave_id = **wave_ids.first()?;
...
mod tests {
    ...
    fn test_unknown_topic_returns_none() {
        let registry = make_registry_with_concurrent_hat();
        let events = vec![
            make_wave_event("unknown.topic", "payload", "w-abc", 0, 1),
        ];
