...
- **Embedded presets**: `import:` is not supported in `-H builtin:<name>` presets, which have no filesystem context.
...
| `instructions` | string | Yes | Prompt injected when hat is active |
| `extra_instructions` | string | No | Additional instructions appended |
