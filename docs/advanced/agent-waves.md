...
Two wave-enabled presets ship with Ralph:
...
| `wave-research` | `presets/wave-research.yml` | Parallel research with synthesis | Researcher | 3 |
...
# Parallel research
ralph run -c ralph.yml -H presets/wave-research.yml -p "Research error handling patterns in Rust async"
```
...
- **One wave per iteration** — If multiple waves are detected, only the first is executed
